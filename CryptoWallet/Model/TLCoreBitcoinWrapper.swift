...
class TLCoreBitcoinWrapper {
    ...
    class func getBIP44KeyChain(_ masterHex:NSString) -> BTCKeychain{
        ...
        let accountKeychain = coinTypeKeychain?.derivedKeychain(at: 0, hardened:true)
    ...
    class func getExtendPubKeyFromMasterHex(_ masterHex:String, accountIdx:UInt) -> String{
        let accountKeychain = getBIP44KeyChain(masterHex as NSString)
        return accountKeychain.extendedPublicKey
    }
    
    class func getExtendPrivKey(_ masterHex:String) -> String{
        let accountKeychain = getBIP44KeyChain(masterHex as NSString)
    ...
    class func getStandardPubKeyHashScriptFromAddress(_ address:String, isTestnet:Bool) -> String {
        let scriptData = BTCScript(address: BTCAddress(string: address))
