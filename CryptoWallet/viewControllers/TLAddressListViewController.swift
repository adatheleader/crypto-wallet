...
//    fileprivate var QRImageModal: TLQRImageModal?
    ...
    let TL_STRING_NO_STEALTH_PAYMENT_ADDRESSES_INFO = "This account type can't see reusable address payments"
...
//        self.QRImageModal!.show()
    ...
    func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        if (section == 0) {
            ...
            return "Reusable Address Payment Addresses"
        } else if (section == 1) {
    ...
    func tableView(_ tableView: UITableView, willSelectRowAt indexPath: IndexPath) -> IndexPath? {
        ...
        if (indexPath as NSIndexPath).section == 0 {
            addressType = .stealth
            title = String(format: "Payment Index: %lu", self.accountObject!.stealthWallet!.getStealthAddressPaymentsCount() - (indexPath as NSIndexPath).row)
